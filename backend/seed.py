...
# Ultra-stable, high-availability curated image arrays
...
async def seed_database():
    ...
    print("Re-indexing database schema with cross-origin safe image links...")
    ...
    for category in CATEGORIES:
        ...
        for brand in brands:
            for item in items:
                image_target = STUDIO_PRODUCT_IMAGES.get(item, CATEGORY_FALLBACKS[category])
                ...
                for model_variant in range(1, 3):  
                    ...
                    name = f"{brand} {item} (Model V{model_variant:02d})"
